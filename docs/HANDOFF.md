...
- 스쿠나 Milestone 2 푸가: 구현 완료, 거리·자원·피해 세부 조정 보류
- 스쿠나 Milestone 3 복마어주자: 원격 구현 완료, 사용자 테스트 필요
- 영역 안 푸가 광역 전환: 복마어주자 확인 뒤 `NOW 후보`
...
## 9. 현재 원격 저장소의 새 작업 — 복마어주자 테스트 필요
...
2. 숫자 2로 스쿠나 전환
→ V HUD가 READY · CE 80인지
→ CE 시작값 95 / 160인지

3. 적 두 마리가 살아 있는 상태에서 V
→ 약 0.65초 준비
→ 준비 중 이동이 느려지는지
→ 준비 중 Q/E/R/LMB/SPACE 새 입력이 막히는지

4. 복마어주자 전개
→ 밀폐된 벽 없이 중앙 사당과 지면 범위선 생성
→ 적 두 마리 모두 교차 참격을 반복해서 받는지
→ 총 8회 참격인지
→ 각 적이 풀 HP였다면 100 → 약 4까지 감소하는지

5. 전개 중
→ Q/E/R, 기본 공격과 새 회피가 차단되는지
→ WASD 이동은 가능한지

6. 약 3.2초 뒤
→ 사당과 지면선이 사라지는지
→ V 쿨타임 표시
→ Q/E/R과 기본 공격·회피가 다시 가능한지

7. R 푸가 거리 조건은 그대로 유지되는지
...
- Unity에서 직접 컴파일하지 않았으므로 첫 테스트에서 Console 확인 필요
...
```text
1. 컴파일·판정·HUD·VFX 회귀 오류 수정
2. 영역 수치와 행동 잠금 체감 평가
3. 스쿠나 전용 영역 사운드 분리
4. 영역 안 푸가의 광역 전환 조건 조사·구현
5. 영역 밖/안 푸가 대상 규칙 비교
6. 그 뒤 영역 후 상태 변화나 공통 영역 구조 추출 여부 판단
```
...
1. 이 문서를 읽는다.
2. 고죠 전투와 스쿠나 해·팔은 사용자 확인 완료임을 유지한다.
3. 사용자가 9번 복마어주자를 pull·테스트했는지 확인한다.
4. 오류가 있으면 수정하면서 계획된 다음 작업을 진행한다.
