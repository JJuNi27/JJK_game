...
class SukunaDomainController:
    ...
    def complete_mouse_seal(
        ...
        *,
        ...
        """좌·우클릭이 모두 눌렸을 때 동시성과 제한 시간을 판정한다."""
    ...
    def incomplete_mouse_release(self) -> None:
        """양쪽 마우스 결합 전 버튼이 먼저 풀린 경우 실패 처리한다."""
        if self.state == GameState.WAIT_LEFT_CLICK:
    ...
    def update(self) -> None:
        ...
        if (
            ...
            if self.left_hand_started_at is not None:
                if (
                    now - self.left_hand_started_at
                    > self.config.left_hand_to_mouse_timeout
                ):
                    self._fail("실패: 마우스 장인 결합이 너무 늦었습니다")

        elif self.state == GameState.RELEASE_TIMING:
            if self.mouse_seal_completed_at is not None:
                max_wait = (
