Shader "JJKGame/Candidate/Purple Polish Corona"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 _Centre,_Halo,_Motion;float _RearStretch,_CoronaContrast;
            CBUFFER_END
            ...
            half4 Frag(V i):SV_Target
            {
                ...
                [loop]for(int k=0;k<80;k++)
                {
                    ...
                    float gap=smoothstep(lerp(.35,.43,_CoronaContrast),lerp(.66,.70,_CoronaContrast),large);
                    float thickness=lerp(lerp(.045,.025,_CoronaContrast),lerp(.14,.095,_CoronaContrast),smoothstep(.25,.78,large));
                    ...
                    float rupture=smoothstep(lerp(.27,.38,_CoronaContrast),lerp(.65,.72,_CoronaContrast),tear)*gap;
                    float diffuse=exp(-abs(r-radius+.09)*10)*gap*lerp(.20,.035,_CoronaContrast);
                    sum+=(crest*rupture*lerp(2.8,4.8,_CoronaContrast)+diffuse)*stride;
                }
