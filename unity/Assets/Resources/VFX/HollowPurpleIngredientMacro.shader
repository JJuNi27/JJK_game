Shader "JJKGame/Candidate/Hollow Purple Ingredient Macro"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            V Vert(A i)
            {
                V o;float3 dir=normalize(i.positionOS.xyz);float clock=_PhaseTime*1.8;
            ...
            half4 Frag(V i):SV_Target
            {
                ...
                float t=_PhaseTime*_Polarity*1.85;
                ...
                float3 result=dark+colour*(fissure*3.4+rim*.30*edgeBreak+smoothstep(.48,.76,noise)*.09)*light;
                float voidCut=pow(saturate(1-abs(noise-.39)*13),5)*smoothstep(.43,.68,N(flow*1.4+5));
                result*=1-voidCut*.82;
                ...
                if(red<.5)
                {
                    float sink=exp(-(1-facing)*(18+coreNoise*8));
                    result=lerp(result,float3(.001,.004,.018),sink);
                }
                else
                {
                    float hot=exp(-(1-facing)*(64+(coreNoise-.5)*48));
                    float shoulder=exp(-(1-facing)*13);
                    result+=float3(5,1.5,1.7)*hot*(.82+.18*sin(t*13))+colour*shoulder*.32;
                }
