Shader "JJKGame/Exploration/Purple R2 Tube Arc"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(V i):SV_Target
            {
                ...
                float white=pow(facing,6)*i.color.g;
                float3 light=lerp(float3(2.2,.008,.85),float3(5,2.8,5),white);
                light*=.65+.35*saturate(dot(normalize(i.normal),normalize(float3(-.5,.8,-.3))));
                light*=exp(-through*float3(.7,2.8,1.1));
                float pulse=.8+.2*sin(i.uv.x*47-_PhaseTime*32);
