...
{
    public sealed class PurpleBodyHybridD2R1Tests
    {
        private const string Output="D:/JJK_game/unity/Logs/PurpleD2R1QA";
        private static PurpleBodyHybridD2R1Profile Profile=>Resources.Load<PurpleBodyHybridD2R1Profile>("VFX/PurpleBodyHybridD2R1Profile");
        [UnityTearDown]public IEnumerator Cleanup(){Time.timeScale=1;if(EditorApplication.isPlaying)yield return new ExitPlayMode();}
        private static Texture2D Read(PurpleBodyHybridD2R1Bench bench)
        {
        ...
        private static void Still(PurpleBodyHybridD2R1Bench bench,string file)
        ...
        {
            foreach(string path in new[]{"VFX/HollowPurpleHybridD2R1","VFX/HollowPurpleHybridD2R1Rupture"})
        ...
        [UnityTest] public IEnumerator SafePreviewCompileAndClose()
        {
            ...
            var editor=UnityEditor.Editor.CreateEditor(Profile);Assert.That(editor,Is.TypeOf<PurpleBodyHybridD2R1ProfileEditor>());Object.DestroyImmediate(editor);
            var root=new GameObject("LightweightD2R1PreviewTest");var bench=root.AddComponent<PurpleBodyHybridD2R1Bench>();bench.Configure(Profile,main);bench.Common.BenchmarkCamera.enabled=false;
            string folder=Path.Combine(Output,"Previews",DateTime.UtcNow.ToString("yyyyMMdd_HHmmss_fff"));Directory.CreateDirectory(folder);Debug.Log("D2R1 preview: "+folder);
            bool async=ShaderUtil.allowAsyncCompilation;ShaderUtil.allowAsyncCompilation=false;
            try
            {
                bench.Select(false);bench.Sample(1.5f,PurpleBenchmarkView.Front);Still(bench,Path.Combine(folder,"D2_Front.png"));
                bench.Select(true);bench.Sample(.35f,PurpleBenchmarkView.Front);Still(bench,Path.Combine(folder,"R1_Front_035.png"));
                bench.Sample(1.5f,PurpleBenchmarkView.Front);Still(bench,Path.Combine(folder,"R1_Front.png"));
                bench.Sample(1.5f,PurpleBenchmarkView.Side);Still(bench,Path.Combine(folder,"R1_Side.png"));ShaderCheck();
            }
            ...
            Assert.That(Object.FindFirstObjectByType<PurpleBodyHybridD2R1Prototype>(),Is.Null);
            foreach(var m in Resources.FindObjectsOfTypeAll<Material>())Assert.That(m.name.StartsWith("PurpleHybridD2R1_"),Is.False,"D2R1 material cleanup");
        ...
        [UnityTest]public IEnumerator MinimumStillAndMotion()
        {
            ...
            yield return EditorSceneManager.LoadSceneAsyncInPlayMode("Assets/Scenes/VFXLab.unity",new LoadSceneParameters(LoadSceneMode.Single));
            yield return null;yield return null;yield return null;Time.timeScale=0;
            var root=new GameObject("D2R1MinimumRender");var bench=root.AddComponent<PurpleBodyHybridD2R1Bench>();bench.Configure(Profile,Camera.main);bench.Common.BenchmarkCamera.enabled=false;
            string folder=Path.Combine(Output,"Motion",DateTime.UtcNow.ToString("yyyyMMdd_HHmmss_fff"));Directory.CreateDirectory(folder);Debug.Log("D2R1 motion: "+folder);
            bool async=ShaderUtil.allowAsyncCompilation;ShaderUtil.allowAsyncCompilation=false;
            try
            {
                foreach(var view in new[]{PurpleBenchmarkView.Front,PurpleBenchmarkView.Side,PurpleBenchmarkView.SlightLow})
                {bench.Sample(1.5f,view);Still(bench,Path.Combine(folder,"R1_"+view+".png"));}
