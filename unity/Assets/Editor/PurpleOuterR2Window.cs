...
{
    public sealed class PurpleOuterR2Window : EditorWindow
    {
        private PurpleOuterR2Bench bench;
        [MenuItem("Tools/JJK Game/VFXLab/Purple D2-R3-OuterR2 비교")]
        public static void Open()=>GetWindow<PurpleOuterR2Window>("R3 / OuterR1 / OuterR2");
        ...
        private void Begin()
        {
            var profile=Resources.Load<PurpleOuterR2Profile>("VFX/PurpleOuterR2Profile");
            if(profile==null || profile.frozenR3==null){Debug.LogError("D2-R3-OuterR2 비교 설정이 없습니다.");return;}
            bench=new GameObject("CodexPurpleOuterR2Comparison").AddComponent<PurpleOuterR2Bench>();bench.Configure(profile,Camera.main);
        }
        ...
        private void OnGUI()
        {
            EditorGUILayout.LabelField("R3 / OuterR1 / OuterR2 · 외부 아우라 / 파편 / 방전",EditorStyles.boldLabel);
            ...
            {EditorGUILayout.HelpBox("VFXLab Play Mode에서 시작하세요. 기존 Scene과 R3 본체는 보존됩니다.",MessageType.Info);return;}
            if(bench==null){if(GUILayout.Button("별도 D2-R3-OuterR2 비교 화면 열기",GUILayout.Height(32)))Begin();return;}
            int selected=GUILayout.Toolbar(bench.Selected,new[]{"R3 · 본체 단독","OuterR1 · 동결 기준","OuterR2 · 외부 개선"},GUILayout.Height(28));
            if(selected!=bench.Selected){bench.Playing=false;bench.SelectCandidate(selected);}
            ...
            EditorGUILayout.HelpBox("본체를 가리지 않고 외부 아우라·파편·방전·공간 왜곡이 읽히는지 비교하세요.",MessageType.None);
            EditorGUILayout.BeginHorizontal();
            if(GUILayout.Button("OuterR2 설정 선택"))Selection.activeObject=Resources.Load<PurpleOuterR2Profile>("VFX/PurpleOuterR2Profile");
            if(GUILayout.Button("OuterR2 설정 다시 읽기")){int selectedCandidate=bench.Selected;var v=common.View;var t=common.Clock;CloseBench();Begin();if(bench!=null){bench.SelectCandidate(selectedCandidate);bench.Sample(t,v);}}
            if(GUILayout.Button("비교 종료"))CloseBench();EditorGUILayout.EndHorizontal();
        }
    }
    [CustomEditor(typeof(PurpleOuterR2Profile))]
    public sealed class PurpleOuterR2ProfileEditor : UnityEditor.Editor
    {
        private static readonly System.Collections.Generic.Dictionary<string,string> Labels=new()
        {
            ...
            ["sparkCount"]="파편 최대 수",["sparkSpeed"]="파편 외향 속도",["sparkEmission"]="파편 발광",["heroFragments"]="주요 파편 수",["heroFragmentScale"]="주요 파편 크기",
        ...
        {
            serializedObject.Update();EditorGUILayout.HelpBox("OuterR2 전용 설정입니다. 참조하는 R3 본체 및 이전 후보의 원본 asset은 수정하지 마세요.",MessageType.Info);
