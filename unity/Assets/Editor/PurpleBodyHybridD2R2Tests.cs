...
{
    public sealed class PurpleBodyHybridD2R2Tests
    {
        private const string Output="D:/JJK_game/unity/Logs/PurpleD2R2QA";
        private static PurpleBodyHybridD2R2Profile Profile=>Resources.Load<PurpleBodyHybridD2R2Profile>("VFX/PurpleBodyHybridD2R2Profile");
        [UnityTearDown]public IEnumerator Cleanup(){Time.timeScale=1;if(EditorApplication.isPlaying)yield return new ExitPlayMode();}
        private static Texture2D Read(PurpleBodyHybridD2R2Bench bench)
        {
        ...
        private static void Still(PurpleBodyHybridD2R2Bench bench,string file)
        ...
        {
            foreach(string path in new[]{"VFX/HollowPurpleHybridD2R2","VFX/HollowPurpleHybridD2R2Arc"})
        ...
        [UnityTest] public IEnumerator SafePreviewCompileAndClose()
        {
            ...
            var editor=UnityEditor.Editor.CreateEditor(Profile);Assert.That(editor,Is.TypeOf<PurpleBodyHybridD2R2ProfileEditor>());Object.DestroyImmediate(editor);
            var root=new GameObject("LightweightD2R2PreviewTest");var bench=root.AddComponent<PurpleBodyHybridD2R2Bench>();bench.Configure(Profile,main);bench.Common.BenchmarkCamera.enabled=false;
            string folder=Path.Combine(Output,"Previews",DateTime.UtcNow.ToString("yyyyMMdd_HHmmss_fff"));Directory.CreateDirectory(folder);Debug.Log("D2R2 preview: "+folder);
            bool async=ShaderUtil.allowAsyncCompilation;ShaderUtil.allowAsyncCompilation=false;
            try
            {
                bench.Select(false);bench.Sample(1.5f,PurpleBenchmarkView.Front);Still(bench,Path.Combine(folder,"R1_Front.png"));
                bench.Select(true);bench.Sample(.35f,PurpleBenchmarkView.Front);Still(bench,Path.Combine(folder,"R2_Front_035.png"));
                bench.Sample(1.5f,PurpleBenchmarkView.Front);Still(bench,Path.Combine(folder,"R2_Front.png"));
                bench.Sample(1.5f,PurpleBenchmarkView.Side);Still(bench,Path.Combine(folder,"R2_Side.png"));ShaderCheck();
            }
            ...
            Assert.That(Object.FindFirstObjectByType<PurpleBodyHybridD2R2Prototype>(),Is.Null);
            foreach(var m in Resources.FindObjectsOfTypeAll<Material>())Assert.That(m.name.StartsWith("PurpleHybridD2R2_"),Is.False,"D2R2 material cleanup");
        ...
        [UnityTest]public IEnumerator MinimumStillAndMotion()
        {
            ...
            yield return EditorSceneManager.LoadSceneAsyncInPlayMode("Assets/Scenes/VFXLab.unity",new LoadSceneParameters(LoadSceneMode.Single));
            yield return null;yield return null;yield return null;Time.timeScale=0;
            var root=new GameObject("D2R2MinimumRender");var bench=root.AddComponent<PurpleBodyHybridD2R2Bench>();bench.Configure(Profile,Camera.main);bench.Common.BenchmarkCamera.enabled=false;
            string folder=Path.Combine(Output,"Motion",DateTime.UtcNow.ToString("yyyyMMdd_HHmmss_fff"));Directory.CreateDirectory(folder);Debug.Log("D2R2 motion: "+folder);
            bool async=ShaderUtil.allowAsyncCompilation;ShaderUtil.allowAsyncCompilation=false;
            try
            {
                foreach(var view in new[]{PurpleBenchmarkView.Front,PurpleBenchmarkView.Side,PurpleBenchmarkView.SlightLow})
                {bench.Sample(1.5f,view);Still(bench,Path.Combine(folder,"R2_"+view+".png"));}
