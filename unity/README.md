...
```text
해 사용 1/1 + 팔 사용 1/1 필요
영역 밖 살아 있는 적 정확히 1명 필요
소비 35
준비 약 0.52초
현재 최대 추적 거리 약 24m
단일 대상 피해 78
...
- 적이 두 명 이상이면 발동하지 않고 준비 상태를 보존합니다.
- 추적 화염 발사체가 지정된 한 대상에게 접근해 폭발합니다.
- 폭발은 시각적으로 넓지만 영역 밖 피해는 지정된 한 대상에게만 적용합니다.
- 성공 발동 뒤 해·팔 준비 상태를 소모합니다.
- `DamageContext.CursedTechnique`이라 무하한을 자체적으로 우회하지 않습니다.
- 약 24m 제한은 원작 조건이 아닌 조정 가능한 테스트 수치입니다.
...
- 장벽 벽이나 밀폐 구체를 만들지 않는 개방형 영역
- 발동 위치에 고정된 Primitive 사당과 지면 범위선
- 범위 안 살아 있는 적 모두에게 반복 참격
- `DamageDeliveryType.DomainSureHit`
- 기존 무하한 우회 구조와 연결
- 영역 준비·전개 중 Q/E/R, 기본 공격과 새 회피 차단
- 전개 중 WASD 이동 허용
- V 직접 입력은 Python 장인·음성 인식 연결 전까지의 임시 입력

현재 포함하지 않음:

- 영역 안 푸가 광역 전환
- 팬텀 퍼레이드식 최대 주력 증가·대량 회복
- 영역 후 스킬 변화
- 영역 충돌·상쇄·덮어쓰기
- 완성형 환경 파괴
...
- 푸가는 임시 시전음·폭발음과 카메라 피드백
- 복마어주자는 현재 공통 영역 효과음 재사용
- 승리·패배
